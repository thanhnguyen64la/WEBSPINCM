...
$body = [
    'title' => 'Chỉnh Sửa Dịch Vụ | Quản Lý Website'
];
require_once(__DIR__ . '/../../models/is_admin.php');
$HN = new DATABASE();
$id_service = isset($_GET['id']) ? check_string($_GET['id']) : null;
if ($id_service != null) {
    $service = $HN->get_row("SELECT * FROM `services` WHERE `id` = '$id_service'");
    if ($service === false) {
        die('Dịch vụ không tồn tại trong hệ thống');
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if ($isUpdate) {
        $HN->insert("logs", [
            ...
            'action' => "Chỉnh sửa dịch vụ (" . $_POST['name'] . ")."
        ]);
        msg_success_link('Sửa dịch vụ thành công', base_url("?module=admin&action=services"), 2000);
    } else {
                        ...
                        <div class="card-body">
                            <div class="row mb-5">
                                ...
                                <div class="col-sm-6 mb-2">
                                    ...
                                    <input type="text" class="form-control" value="<?= $service['type']; ?>" name="type"
                                        placeholder="Nhập type dịch vụ" required>
                                </div>
                                <div class="col-sm-6 mb-2">
                                    <label class="form-label">Giá bán: <span class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="text" value="<?= $service['price']; ?>"
                                            class="form-control text-center" name="price" required>
                                ...
                                <div class="col-sm-6 mb-2">
                                    ...
                                    <div class="input-group">
                                        <input type="text" value="<?= $service['api_price']; ?>"
                                            class="form-control text-center" name="api_price" required>
                                ...
                                <div class="col-sm-6 mb-2">
                                    ...
                                    <select class="form-control" name="api_server" required>
                                        <option <?= $service['api_server'] == "MUASPIN" ? "selected" : ""; ?>
                                            value="MUASPIN">MUASPIN</option>
                                    </select>
                                </div>
                                <div class="col-sm-12 mb-2">
                                    <label class="form-label">Trạng thái: <span class="text-danger">*</span></label>
                                    <select class="form-control" name="status" required>
                                        <option <?= $service['status'] == 'on' ? 'selected' : ''; ?> value="on">ON</option>
                                        <option <?= $service['status'] == 'off' ? 'selected' : ''; ?> value="off">OFF
                                        </option>
                                    </select>
                                </div>
                                <div class="col-sm-12 mb-2">
