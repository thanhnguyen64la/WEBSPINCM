...
$head = [
    'title' => 'Biến Động Số Dư' . ' | ' . $HN->setting('title'),
...
$reason = isset($_GET['reason']) ? check_string($_GET['reason']) : '';
$from = ($page - 1) * $limit;
...
$totalRecords = $HN->num_rows("SELECT * FROM `cash_flow` WHERE $where $sqlReal ORDER BY `id` DESC");
$totalPages = ceil($totalRecords / $limit);
$cash_flow = $HN->get_list("SELECT * FROM `cash_flow` WHERE $where $sqlReal ORDER BY `id` DESC LIMIT $limit OFFSET $from");
$pagination = pagination_client(base_url("?action=transactions&reason=$reason&time=$time&limit=$limit&shortByDate=$shortByDate"), $from, $totalRecords, $limit);
?>
                    ...
                    <form method="GET" class="mb-3">
                        ...
                        <div class="row">
                            <div class="col-lg col-md-4 col-6">
                                <input class="form-control mb-2" type="text" value="<?= $reason ?>" name="reason"
                                    placeholder="Lý do">
                            </div>
                            <div class="col-lg col-md-4 col-6">
                            ...
                            <div class="col-lg col-md-4 col-6">
                                <a href="<?= base_url("?module=client&action=transactions") ?>"
                                    class="shop-widget-btn mb-2"><i class="far fa-trash-alt"></i><span>Bỏ lọc</span></a>
                            </div>
                        </div>
                        ...
                        <table class="table fs-sm mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Số tiền ban đầu</th>
                                    <th class="text-center">Số tiền thay đổi</th>
                                    <th class="text-center">Số tiền hiện tại</th>
                                    <th>Lý do</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($cash_flow as $row) { ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <b style="color: green;"><?= format_currency($row['initial_amount']); ?></b>
                                        </td>
                                        <td class="text-center">
                                            <b style="color: red;"><?= format_currency($row['changed_amount']); ?></b>
                                        </td>
                                        <td class="text-center">
                                            <b style="color: blue;"><?= format_currency($row['current_amount']); ?></b>
                                        </td>
                                        <td>
